 ...
 * @param "From" (optional.String) -  Search for messages after this date. Use ISO-8601 normalization.
 * @param "To" (optional.String) -  Search for messages before this date. Use ISO-8601 normalization.
 * @param "Offset" (optional.Int32) -  Offset from the first result you want to fetch. offset + limit should not exceed 10.000.
 * @param "Limit" (optional.Int32) -  Maximum amount of messages to return. limit should not exceed 1.000.
 * @param "Sort" (optional.String) -  Sort order, based on timestamp field of the AuditLog message.
 * @param "Filters" (optional.String) -  Filter query based on parameter name = field path and parameter value = value to search for : {fiedlName}={value}. You can put several filters that way, they will all be treated with an AND operator. Common filters field names you can use : level, category, subcategory, type, source.deviceId, source.nodeId, description, detailedDescription. e.g. : level=error&source.deviceId=urn:lora:0E5EAB0ABCD00000
 * @param "Any" (optional.Interface of []string) -  Search for AuditLog Messages where any of the fields contains all these values.
@return []AuditLogMessage
*/
func (a *AuditLogApiService) SearchUsingGET(ctx _context.Context, xAPIKEY string, localVarOptionals *SearchUsingGETOpts) ([]AuditLogMessage, *_nethttp.Response, error) {
	var (
	...
	localVarPath := a.client.cfg.BasePath + "/api/v0/auditlog/messages"
