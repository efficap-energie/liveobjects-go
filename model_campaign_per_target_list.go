/*
 * Live Objects REST API Guide v2.13.3
 *
 * API description for Live Objects service
 *
 * API version: 2.13.3
 * Contact: liveobjects.support@orange.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package liveobjects

import (
	"encoding/json"
)

// CampaignPerTargetList struct for CampaignPerTargetList
type CampaignPerTargetList struct {
	// number of the current page: starts at 0.
	Page int64 `json:"page"`
	// number of data per page (= maximum number of data in the associated list of data:the last page can have less data)
	Size int64 `json:"size"`
	// total count of data in the complete list.
	TotalCount int64 `json:"totalCount"`
	// list of data in this page.
	Data []CampaignPerTarget `json:"data"`
}

// NewCampaignPerTargetList instantiates a new CampaignPerTargetList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCampaignPerTargetList(page int64, size int64, totalCount int64, data []CampaignPerTarget, ) *CampaignPerTargetList {
	this := CampaignPerTargetList{}
	this.Page = page
	this.Size = size
	this.TotalCount = totalCount
	this.Data = data
	return &this
}

// NewCampaignPerTargetListWithDefaults instantiates a new CampaignPerTargetList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCampaignPerTargetListWithDefaults() *CampaignPerTargetList {
	this := CampaignPerTargetList{}
	return &this
}

// GetPage returns the Page field value
func (o *CampaignPerTargetList) GetPage() int64 {
	if o == nil  {
		var ret int64
		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *CampaignPerTargetList) GetPageOk() (*int64, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Page, true
}

// SetPage sets field value
func (o *CampaignPerTargetList) SetPage(v int64) {
	o.Page = v
}

// GetSize returns the Size field value
func (o *CampaignPerTargetList) GetSize() int64 {
	if o == nil  {
		var ret int64
		return ret
	}

	return o.Size
}

// GetSizeOk returns a tuple with the Size field value
// and a boolean to check if the value has been set.
func (o *CampaignPerTargetList) GetSizeOk() (*int64, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Size, true
}

// SetSize sets field value
func (o *CampaignPerTargetList) SetSize(v int64) {
	o.Size = v
}

// GetTotalCount returns the TotalCount field value
func (o *CampaignPerTargetList) GetTotalCount() int64 {
	if o == nil  {
		var ret int64
		return ret
	}

	return o.TotalCount
}

// GetTotalCountOk returns a tuple with the TotalCount field value
// and a boolean to check if the value has been set.
func (o *CampaignPerTargetList) GetTotalCountOk() (*int64, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.TotalCount, true
}

// SetTotalCount sets field value
func (o *CampaignPerTargetList) SetTotalCount(v int64) {
	o.TotalCount = v
}

// GetData returns the Data field value
func (o *CampaignPerTargetList) GetData() []CampaignPerTarget {
	if o == nil  {
		var ret []CampaignPerTarget
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *CampaignPerTargetList) GetDataOk() (*[]CampaignPerTarget, bool) {
	if o == nil  {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *CampaignPerTargetList) SetData(v []CampaignPerTarget) {
	o.Data = v
}

func (o CampaignPerTargetList) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["page"] = o.Page
	}
	if true {
		toSerialize["size"] = o.Size
	}
	if true {
		toSerialize["totalCount"] = o.TotalCount
	}
	if true {
		toSerialize["data"] = o.Data
	}
	return json.Marshal(toSerialize)
}

type NullableCampaignPerTargetList struct {
	value *CampaignPerTargetList
	isSet bool
}

func (v NullableCampaignPerTargetList) Get() *CampaignPerTargetList {
	return v.value
}

func (v *NullableCampaignPerTargetList) Set(val *CampaignPerTargetList) {
	v.value = val
	v.isSet = true
}

func (v NullableCampaignPerTargetList) IsSet() bool {
	return v.isSet
}

func (v *NullableCampaignPerTargetList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCampaignPerTargetList(val *CampaignPerTargetList) *NullableCampaignPerTargetList {
	return &NullableCampaignPerTargetList{value: val, isSet: true}
}

func (v NullableCampaignPerTargetList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCampaignPerTargetList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


